...
import { View, Text, StyleSheet, Image, Platform, FlatList, TouchableOpacity, ActivityIndicator, Linking, Alert, Animated, Easing, Dimensions } from 'react-native';
...
const NewsScreen = () => {
  ...
  const renderArticle = ({ item }: { item: Article }) => (
    <TouchableOpacity style={styles.articleRow} onPress={() => openUrl(item.url)}>
      {item.urlToImage ? (
      ...
      <View style={styles.articleContent}>
        ...
        <Text style={styles.articleDescription} numberOfLines={3}>
          {item.description ? item.description.slice(0, 100) + (item.description.length > 100 ? '...' : '') : 'No description available.'}
        </Text>
        <Text style={styles.articleMeta}>
          {item.author ? `By ${item.author} • ` : ''}{new Date(item.publishedAt).toLocaleDateString()}
        </Text>
      </View>
    </TouchableOpacity>
  );
  ...
  return (
    <View style={styles.gradientBg}>
      <View style={styles.bannerContainer}>
        ...
        />
        <Text style={styles.bannerText}>Stay Informed</Text>
      </View>
      ...
      <View style={styles.contentContainer}>
        <Text style={styles.title}>Financial Inclusion News</Text>
        {loading && articles.length === 0 ? (
          ...
          <FlatList
            ...
            ListFooterComponent={
              hasMore ? (
                loadingMore ? (
                  <ActivityIndicator size="small" color="#008080" style={{ marginVertical: 16 }} />
                ) : (
                  <Animated.View style={{ transform: [{ scale: loadMoreScale }] }}>
