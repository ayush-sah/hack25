...
import { View, TouchableOpacity, StyleSheet, Image, Animated, Dimensions, FlatList, Easing, Modal, TextInput, ScrollView } from 'react-native';
...
const categoryIcons: Record<string, JSX.Element> = {
  'General': <MaterialCommunityIcons name="finance" size={32} color="#DE3163" />,
  'Barriers': <MaterialCommunityIcons name="block-helper" size={32} color="#E67E22" />,
  'Consequences': <MaterialCommunityIcons name="alert-circle" size={32} color="#E74C3C" />,
  'Solutions': <MaterialCommunityIcons name="lightbulb-on" size={32} color="#27AE60" />,
  'Banking Basics': <FontAwesome5 name="university" size={32} color="#2980B9" />,
  'Saving & Budgeting': <FontAwesome5 name="piggy-bank" size={32} color="#F39C12" />,
  'Credit & Loans': <FontAwesome5 name="credit-card" size={32} color="#8E44AD" />,
  'Digital Finance': <Ionicons name="phone-portrait" size={32} color="#16A085" />,
  'Investing': <FontAwesome name="line-chart" size={32} color="#2ECC71" />,
  'Insurance': <MaterialCommunityIcons name="shield-check" size={32} color="#34495E" />,
  'Retirement Planning': <FontAwesome5 name="user-clock" size={32} color="#C0392B" />,
  'Taxes': <FontAwesome5 name="file-invoice-dollar" size={32} color="#27AE60" />,
};
...
const allQuestions = [
  ...
  {
    category: 'Solutions',
    question: 'What is a government policy that can promote financial inclusion?',
    options: [
...
export default function TriviaGame() {
  ...
  const handleCategorySelect = async (cat: string) => {
    setCategory(cat);
    setQuestions(allQuestions.filter(q => q.category === cat));
    setCurrent(0);
    setScore(0);
    setShowResult(false);
    setSelected(null);
  ...
  // Add user question to the current category
  const handleAddQuestion = () => {
    ...
    setUserQuestions(prev => [
      ...prev,
      {
        ...
        answer: newQ.answer,
        image: require('../../assets/images/faq.png'),
        fact: newQ.fact || 'User contributed question.',
      },
    ]);
    setShowAddModal(false);
    setNewQ({ question: '', options: ['', '', '', ''], answer: 0, fact: '' });
  };

  // Merge user questions with allQuestions for the current category
  const questionsToDisplay = React.useMemo(() => {
    if (!category) return [];
    return [
      ...allQuestions.filter(q => q.category === category),
      ...userQuestions.filter(q => q.category === category),
    ];
  ...
  if (!category) {
    ...
    const renderCategory = ({ item }: { item: { key: string; label: string } }) => (
      <TouchableOpacity
        ...
        style={[
          styles.categoryBtn,
          selectedCat === item.key && styles.categoryBtnSelected,
        ]}
        activeOpacity={0.8}
      ...
      >
        <View style={styles.categoryIcon}>{categoryIcons[item.key]}</View>
        <ThemedText style={styles.categoryText}>{item.label}</ThemedText>
      </TouchableOpacity>
    );
    return (
      <ThemedView style={styles.container}>
        <ThemedText type="title" style={styles.title}>Select a Category</ThemedText>
        <FlatList
          data={categories}
        ...
        />
        {selectedCat && (
          <TouchableOpacity style={styles.startBtn} onPress={() => handleCategorySelect(selectedCat)}>
    ...
    return (
      <ThemedView style={styles.container}>
        <View style={styles.bannerRow}>
          ...
          <ThemedText type="title" style={styles.title}>{category} Complete!</ThemedText>
        </View>
        <Image source={score >= Math.ceil(questionsToDisplay.length/2) ? correctImg : incorrectImg} style={styles.resultImg} />
        <ThemedText style={styles.score}>Your Score: {score} / {questionsToDisplay.length}</ThemedText>
    ...
    <ThemedView style={styles.container}>
      {showStreak && (
        <View style={styles.streakBanner}>
          <ThemedText style={styles.streakText}>🔥 {streak} Correct in a Row!</ThemedText>
        </View>
      )}
      <View style={styles.bannerRow}>
        {category && categoryIcons[category]}
        <ThemedText type="title" style={styles.title}>{category} Trivia</ThemedText>
      </View>
      <View style={styles.progressBarBg}>
        <Animated.View style={[styles.progressBar, { width: progress.interpolate({ inputRange: [0, 1], outputRange: [0, SCREEN_WIDTH - 80] }) }]} />
      </View>
      <Animated.View style={[styles.card, { opacity: fadeAnim }]}>
        <Image source={feedbackImg ? feedbackImg : questionsToDisplay[current].image} style={styles.questionImg} />
        <ThemedText style={styles.question}>{questionsToDisplay[current].question}</ThemedText>
        <View style={styles.timerRow}>
          <ThemedText style={[styles.timer, timer <= 5 && { color: '#E74C3C' }]}>{timer}s</ThemedText>
        </View>
        {questionsToDisplay[current].options.map((option: string, idx: number) => {
          let optionStyle = styles.option;
          if (selected !== null) {
            if (idx === questionsToDisplay[current].answer) {
              optionStyle = Object.assign({}, styles.option, styles.correct);
            } else if (idx === selected && selected !== questionsToDisplay[current].answer) {
              optionStyle = Object.assign({}, styles.option, styles.incorrect);
            }
          }
          return (
            <TouchableOpacity
        ...
        >
          <View style={{
            ...
            <View style={{
              ...
              <ThemedText style={{ fontSize: 22, fontWeight: 'bold', marginBottom: 10 }}>
        ...
        {selected !== null && !showAddModal && (
          <TouchableOpacity style={styles.addQBtn} onPress={() => setShowAddModal(true)}>
            <ThemedText style={styles.addQBtnText}>Add Your Own Question</ThemedText>
          </TouchableOpacity>
        )}
        <Modal visible={showAddModal} animationType="slide" transparent>
          <View style={styles.modalOverlay}>
            <View style={styles.modalContent}>
              <ScrollView>
                <ThemedText type="title" style={{ marginBottom: 10 }}>Add a New Question</ThemedText>
                <TextInput
                  style={styles.input}
                  placeholder="Question"
                  value={newQ.question}
                  onChangeText={t => setNewQ(q => ({ ...q, question: t }))}
                />
                {newQ.options.map((opt, i) => (
                  <TextInput
                    ...
                    value={opt}
                    onChangeText={t => setNewQ(q => {
                      const opts = [...q.options]; opts[i] = t; return { ...q, options: opts };
                    })}
                  />
                ))}
                <ThemedText style={{ marginTop: 8 }}>Select Correct Option:</ThemedText>
                <View style={styles.answerRow}>
                  {newQ.options.map((opt, i) => (
                    <TouchableOpacity
                      key={i}
                      style={[styles.answerBtn, newQ.answer === i && styles.answerBtnSelected]}
                      onPress={() => setNewQ(q => ({ ...q, answer: i }))}
                    >
                      <ThemedText>{String.fromCharCode(65 + i)}</ThemedText>
                    </TouchableOpacity>
                  ))}
                </View>
                <TextInput
                  style={styles.input}
                  placeholder="Fact (optional)"
                  value={newQ.fact}
                  onChangeText={t => setNewQ(q => ({ ...q, fact: t }))}
                />
                <View style={styles.modalBtnRow}>
                  <TouchableOpacity style={styles.modalBtn} onPress={handleAddQuestion}>
                    <ThemedText style={styles.modalBtnText}>Submit</ThemedText>
                  </TouchableOpacity>
                  <TouchableOpacity style={[styles.modalBtn, { backgroundColor: '#ccc' }]} onPress={() => setShowAddModal(false)}>
