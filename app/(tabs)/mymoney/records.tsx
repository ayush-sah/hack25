                ...
                {state.records.length > 5 && (
                  <TouchableOpacity
                    onPress={() =>
                      Alert.alert(
                        "Info",
                        "Full transaction history can be viewed in a detailed view (coming soon)."
                      )
                    }
                  >
        ...
        <View style={styles.formContainer}>
          ...
          <View style={styles.pickerContainer}>
            <Picker
              ...
              onValueChange={(itemValue) => {
                setType(itemValue);
                setCategoryId(
                  state.categories.filter((c) => c.type === itemValue).length >
                    0
                    ? state.categories.filter((c) => c.type === itemValue)[0].id
                    : ""
                );
              }}
              style={styles.picker}
          ...
          <View style={styles.pickerContainer}>
            ...
            >
              {state.categories.filter((c) => c.type === type).length === 0 ? (
                <Picker.Item
                  label={`No ${type} categories available`}
                  value=""
                />
              ) : (
                state.categories
